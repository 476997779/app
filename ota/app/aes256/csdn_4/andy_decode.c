#include <stdio.h>
//#include <pbt.h>
#include <ctype.h>
#include <unistd.h>
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>

void aes(char*, char*, char*, int);
void aes_detail(int[4][4], int[4][4], int);
void subBytes(int [4][4], int);
void shiftRows(int [4][4], int);
void mixColumns(int [4][4], int);
void addRoundKey(int [4][4], int[4][4]);
int aes_multiple(int, int);
void keyExpansion(int key[4][4], int w[11][4][4]);
int c2i(char );

/**
 * S盒
 */
static const int S_BOX[16][16] = {  };

/**
 * 逆S盒
 */
static const int INVERSE_S_BOX[16][16] = {  };
int RC[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};
#if 1
int main(){
	int method = 0;//1表示加密， 0表示解密
	//待加密/解密文件存放路径
	char * source_path = "./update.zip";
	// 加密/解密后文件存放路径
	char *des_path = "./update.zip";
	// 32位16进制密钥
	char * password = "0f1571c947d9e8590cb7add6af7f6798";
	aes(source_path, des_path, password, method);
	printf("success!!!!!!!!!!");

}
#endif

int	calibrate_size(char *encry, int *size)
{
	int ret;
	int i;
	printf("%s,%d\n",__func__,__LINE__);

	if((ret = memcmp(encry ,"qqqqqqqqqqqqqqqqq",16)) == 0){
		*size = 0;
		return 0;
	}
	if((ret = memcmp(encry+1,"ppppppppppppppp",15)) == 0){
		*size = 1;
		return 0;
	}
	if((ret = memcmp(encry+2,"oooooooooooooo" ,14)) == 0){
		*size = 2;
		return 0;
	}
	if((ret = memcmp(encry+3,"nnnnnnnnnnnnn"  ,13)) == 0){
		*size = 3;
		return 0;
	}
	if((ret = memcmp(encry+4,"mmmmmmmmmmmm"   ,12)) == 0){
		*size = 4;
		return 0;
	}
	if((ret = memcmp(encry+5,"lllllllllll"    ,11)) == 0){
		*size = 5;
		return 0;
	}
	if((ret = memcmp(encry+6,"kkkkkkkkkk"     ,10)) == 0){
		*size = 6;
		return 0;
	}
	if((ret = memcmp(encry+7,"jjjjjjjjj"      ,9)) == 0){
		*size = 7;
		return 0;
	}
	if((ret = memcmp(encry+8,"iiiiiiii"       ,8)) == 0){
		*size = 8;
		return 0;
	}
	if((ret = memcmp(encry+9,"hhhhhhh"        ,7)) == 0){
		*size = 9;
		return 0;
	}
	if((ret = memcmp(encry+10,"gggggg"        ,6)) == 0){
		*size = 10;
		return 0;
	}
	if((ret = memcmp(encry+11,"fffff"         ,5)) == 0){
		*size = 11;
		return 0;
	}
	if((ret = memcmp(encry+12,"eeee"          ,4)) == 0){
		*size = 12;
		return 0;
	}
	if((ret = memcmp(encry+13,"ddd"           ,3)) == 0){
		*size = 13;
		return 0;
	}
	if((ret = memcmp(encry+14,"cc"            ,2)) == 0){
		*size = 14;
		return 0;
	}
	if((ret = memcmp(encry+15,"b"             ,1)) == 0){
		*size = 15;
		return 0;
	}
	return 0;

}

void aes(char* source_path, char* des_path, char* password, int method){
	//将密钥转换成4*4数组
	int p[4][4];
	int m,i,indx;
	int current;
	int pack_num;
	for (m = 0; m < 4; ++m) {
		for (i = 0; i < 4; ++i) {
			indx = 4 * i + m;
			p[i][m] = 16 * c2i(password[indx]) + c2i(password[indx + 1]);
		}
	}

	FILE *file = fopen(source_path, "r+"); //获取文件的指针
	//FILE *file1 = fopen(des_path, "a+");

	int len;
	int size = 16;

	unsigned char content[size];
	unsigned char encry[size];
	int content_to_int[4][4];

	int j, k;
	int remainder;	
	int ret;
	int read_num = 0;
	int flag = 0;
	
	fseek(file,0,SEEK_END);
	current = ftell(file);
	fseek(file,0,SEEK_SET);

	pack_num = current / size;
	printf("pack_num is %d\n",pack_num);

	while((len = fread(content, 1, size, file)) != 0){
		//sleep(3);
		read_num ++;
		
		current = ftell(file);
		
		if ((remainder = len % 16) != 0) {
			printf("%s, %d \n",__func__,__LINE__);
			size = (len / 16 + 1) * 16;
			flag = 1;
		}

		for (j = len; j < size; ++j) {
			printf("yushu is %d\n",16 - remainder);
			content[j] = 'a'+ (16 - remainder);
		}

		for (i = 0; i < size / 16; ++i) {

			for (j = 0; j < 4; ++j) {
				for (k = 0; k < 4; ++k) {
					content_to_int[j][k] = content[j * 4 + k + 16 * i];
				}
			}
			aes_detail(content_to_int, p, method);
			for (j = 0; j < 4; ++j) {
				for (k = 0; k < 4; ++k) {
					encry[j * 4 + k + 16 * i] = content_to_int[j][k];
				}
			}
		}
		
		fseek(file,current-size,SEEK_SET);

		if((0 == method) && (read_num == pack_num)){
			printf("%s,%d\n",__func__,__LINE__);
			calibrate_size(encry,&size);
			
			fwrite(encry, 1, size, file);
			
			fseek(file,current - 16 + size,SEEK_SET);
			
			fclose(file);
			
			int fd = open(des_path, O_RDWR,0666);
			printf("fd is %d\n",fd);
			ftruncate(fd,pack_num * 16 -16 + size);
			close(fd);
			return ;
		}
		
		fwrite(encry, 1, size, file);
		
		fseek(file,current,SEEK_SET);
		//fflush(file1);
	}   
	if((1 == method) && (0 == flag)){
		printf("%s,%d\n",__func__,__LINE__);
		for (j = 0; j < 4; ++j) {
			for (k = 0; k < 4; ++k) {
				content_to_int[j][k] = 'q';
			}
		}
	
		aes_detail(content_to_int, p, method);
			for (j = 0; j < 4; ++j) {
				for (k = 0; k < 4; ++k) {
					encry[j * 4 + k] = content_to_int[j][k];
				}
			}
		fwrite(encry, 1, 16, file);
	}

	fclose(file);
	//fclose(file1);

	printf("%s, %d \n",__func__,__LINE__);

}


void aes_detail(int content[4][4],  int password[4][4], int encode){
	int p[11][4][4];
	keyExpansion(password, p);
	int i;
	if (encode) {
		addRoundKey(content, p[0]);
		for (i = 1; i <= 10; ++i) {
			subBytes(content, encode);
			shiftRows(content, encode);
			if (i != 10) {
				mixColumns(content, encode);
			}

			addRoundKey(content, p[i]);
		}
	}else {
		addRoundKey(content, p[10]);
		for (i = 9; i >= 0; --i) {
			shiftRows(content, encode);
			subBytes(content, encode);
			addRoundKey(content, p[i]);
			if (i != 0) {
				mixColumns(content, encode);
			}
		}
	}
}

void subBytes(int a[4][4], int encode){
	// encode 为1 代表字节替代，为0代表逆向字节替代
	int i,j;
	for (i = 0; i < 4; ++i) {
		for (j = 0; j < 4; ++j) {
			int temp = a[i][j];
			int row = temp / 16;
			int column = temp % 16;
			if (encode)
				a[i][j] = S_BOX[row][column];
			else
				a[i][j] = INVERSE_S_BOX[row][column];
		}
	}
}

void shiftRows(int a[4][4], int encode){
	//encode 为1代表行移位，为0代表逆向行移位
	int i,j;
	for (i = 0; i < 4; ++i) {
		for (j = 0; j < i; ++j) {
			if (encode) {
				int temp = a[i][0];
				a[i][0] = a[i][1];
				a[i][1] = a[i][2];
				a[i][2] = a[i][3];
				a[i][3] = temp;
			} else{
				int temp = a[i][3];
				a[i][3] = a[i][2];
				a[i][2] = a[i][1];
				a[i][1] = a[i][0];
				a[i][0] = temp;
			}
		}
	}
}

void mixColumns(int a[4][4], int encode){
	//encode 为1代表列混淆，为0代表逆向列混淆
	int i, j;
	for ( i = 0; i < 4; ++i) {
		int temp0 = a[0][i];
		int temp1 = a[1][i];
		int temp2 = a[2][i];
		int temp3 = a[3][i];
		if (encode) {
			a[0][i] = aes_multiple(temp0, 2) ^ aes_multiple(temp1, 3) ^ temp2 ^ temp3;
			a[1][i] = temp0 ^ (aes_multiple(temp1, 2)) ^ (temp2 ^ aes_multiple(temp2, 2)) ^ temp3;
			a[2][i] = temp0 ^ temp1 ^ (aes_multiple(temp2, 2)) ^ (temp3 ^ aes_multiple(temp3, 2));
			a[3][i] = temp0 ^ (aes_multiple(temp0, 2)) ^ temp1 ^ temp2 ^ aes_multiple(temp3, 2);
		}else{
			a[0][i] = aes_multiple(temp0, 14) ^ aes_multiple(temp1, 11) ^ aes_multiple(temp2, 13) ^ aes_multiple(temp3, 9);
			a[1][i] = aes_multiple(temp0, 9) ^ aes_multiple(temp1, 14) ^ aes_multiple(temp2, 11) ^ aes_multiple(temp3, 13);
			a[2][i] = aes_multiple(temp0, 13) ^ aes_multiple(temp1, 9) ^ aes_multiple(temp2, 14) ^ aes_multiple(temp3, 11);
			a[3][i] = aes_multiple(temp0, 11) ^ aes_multiple(temp1, 13) ^ aes_multiple(temp2, 9) ^ aes_multiple(temp3, 14);
		}
	}
}

void addRoundKey(int a[4][4], int k[4][4]){
	// 由于用w[11][4][4]表示W[44]导致行列转置，所以在进行异或操作的时候应该是a[i，j] 异或 k[j,i]
	int i, j;
	for (i = 0; i < 4; ++i) {
		for (j = 0; j < 4; ++j) {
			a[i][j] = a[i][j] ^ k[j][i];
		}
	}
}

//AES乘法计算
int aes_multiple(int a, int le){
	int i;
	int thr = le & 0x8;
	int sec = le & 0x4;
	int fir = le & 0x2;
	int fir_mod = le % 2;
	int result = 0;
	if (thr){
		int b = a;
		for (i = 1; i <=3 ; ++i) {
			b = b<<1;
			if (b >= 256)
				b = b ^ 0x11b;
		}
		b = b % 256;
		result = result ^ b;
	}
	if (sec){
		int b = a;
		for (i = 1; i <=2 ; ++i) {
			b = b<<1;
			if (b >= 256)
				b = b ^ 0x11b;
		}
		b = b % 256;
		result = result ^ b;
	}
	if (fir){
		int b = a << 1;
		if (b >= 256)
			b = b ^ 0x11b;
		b = b % 256;
		result = result ^ b;
	}
	if (fir_mod)
		result = result ^ a;
	return  result;
}

void keyExpansion(int key[4][4], int w[11][4][4]){
	int i, j ,k ,l;
	for (i = 0; i < 4; ++i) {
		for (j = 0; j < 4; ++j) {
			w[0][i][j] = key[j][i];
		}
	}
	for (i = 1; i < 11; ++i){
		for (j = 0; j < 4; ++j) {
			int temp[4];
			if (j == 0){
				temp[0] = w[i-1][3][1];
				temp[1] = w[i-1][3][2];
				temp[2] = w[i-1][3][3];
				temp[3] = w[i-1][3][0];
				for (k = 0; k < 4; ++k) {
					int m = temp[k];
					int row = m / 16;
					int column = m % 16;
					temp[k] = S_BOX[row][column];
					if (k == 0){
						temp[k] = temp[k] ^ RC[i-1];
					}
				}
			} else{
				temp[0] = w[i][j-1][0];
				temp[1] = w[i][j-1][1];
				temp[2] = w[i][j-1][2];
				temp[3] = w[i][j-1][3];
			}
			for (l = 0; l < 4; ++l) {

				w[i][j][l] = w[i-1][j][l] ^ temp[l];
			}

		}
	}

}

//将字符转换为数值
int c2i(char ch) {
	// 如果是数字，则用数字的ASCII码减去48, 如果ch = '2' ,则 '2' - 48 = 2
	if(isdigit(ch))
		return ch - 48;

	// 如果是字母，但不是A~F,a~f则返回
	if( ch < 'A' || (ch > 'F' && ch < 'a') || ch > 'z' )
		return -1;

	// 如果是大写字母，则用数字的ASCII码减去55, 如果ch = 'A' ,则 'A' - 55 = 10
	// 如果是小写字母，则用数字的ASCII码减去87, 如果ch = 'a' ,则 'a' - 87 = 10
	if(isalpha(ch))
		return isupper(ch) ? ch - 55 : ch - 87;

	return -1;
}
